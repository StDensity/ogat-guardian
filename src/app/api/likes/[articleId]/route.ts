...
export async function POST(
  ...
  const { articleId } = await params;
  const { client_hash, title, titleUrl } = await request.json();

  if (!articleId || !client_hash) {
    return NextResponse.json(
      { error: "articleId and client_hash required" },
      { status: 400 }
    );
  }

  await sql`
    INSERT INTO likes (article_id, client_hash)
    VALUES (${articleId}, ${client_hash})
    ON CONFLICT (article_id, client_hash) DO NOTHING
  `;
...
export async function DELETE(
  ...
  const { articleId } = await context.params; // <- await here
  const { client_hash } = await request.json();

  if (!client_hash) {
    return NextResponse.json({ error: "client_hash is required" }, { status: 400 });
  }
